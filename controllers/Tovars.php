...
class Tovars extends Controller
{
	...
	public function __construct()
	{
		...
		$this->newsModel = new \models\Tovars();
	...
	public function actionEdit()
	{
		...
		$news = $this->newsModel->GetNewsById($id);
		$titleForbidden = 'Доступ заборонено';
		if (empty($this->user) || $news['user_id'] != $this->userModel->GetCurrentUser()['id'] && $this->userModel->GetCurrentUser()['access'] != 1)
		{
			return $this->render('forbidden', null,
				[
					'MainTitle' => $titleForbidden,
		...
		if($this->isPost())
			{
				...
				return $this->renderMessage('ok', 'Новину редаговано', null,
					[
						'MainTitle' => $title,
						'PageTitle' => $title,
					]); 
